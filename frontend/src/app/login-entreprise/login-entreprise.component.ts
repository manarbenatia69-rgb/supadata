...
@Component({
  ...
  styleUrls: ['./login-entreprise.component.css', '../shared/login-shared.css']
})
export class LoginEntrepriseComponent {
  ...
  login() {
    ...
    let entreprise = new REntreprise(null, null, null,null,null,null,null,null,
          data.email, data.pwd,
...
console.log(entreprise)
    this.service.loginEntreprise(entreprise).subscribe({
      next: (res) => {
        let token = res.token;
        localStorage.setItem("myTokenEntreprise", token);
        ...
        Swal.fire({
          ...
          this.router.navigate(['/fichiercsv']).then(() => {
